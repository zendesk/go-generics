...
func TestFind(t *testing.T) {
	item, wasFound := functions.Find([]string{"foo", "bar", "baz"}, func(f string) bool {
		return f == "foo"
	})
	test.CheckEqual(item, "Test1", "foo", t)
	test.CheckOk(wasFound, "Test1WasFound", t)

	item2, wasFound2 := functions.Find([]int{2, 22, 999}, func(i int) bool {
		return i == 999
	})
	test.CheckEqual(item2, "Test2", 999, t)
	test.CheckOk(wasFound2, "Test2WasFound", t)

	item3, wasFound3 := functions.Find([]float64{2.99, 22, 99.999}, func(i float64) bool {
		return i == 22
	})
	test.CheckEqual(item3, "Test3", 22.0, t)
	test.CheckOk(wasFound3, "Test3WasFound", t)

	item4, notFound1 := functions.Find([]float64{2.99, 22, 99.999}, func(i float64) bool {
		return i == 22.000001
	})
	test.CheckEqual(item4, "Test4", 0.0, t)
	test.CheckOk(!notFound1, "Test4WasFound", t)

	item5, notFound5 := functions.Find([]string{"foo", "bar", "baz"}, func(f string) bool {
		return f == "missingfoo"
	})
	test.CheckEqual(item5, "Test5", "", t)
	test.CheckOk(!notFound5, "Test5WasFound", t)

	item6, notFound6 := functions.Find([]int{2, 22, 999}, func(i int) bool {
		return i == 9991
	})
...
func TestFilter(t *testing.T) {
	...
	negativeNumbers := functions.Filter(ints, func(i int) bool {
		return i < 0
	})

	gt9 := functions.Filter(ints, func(i int) bool {
		return i > 9
	})

	evens := functions.Filter(ints, func(i int) bool {
		return i%2 == 0
	})
...
func TestFilterMap(t *testing.T) {
	...
	newMap := functions.FilterMap(testMap, func(k, v int) bool {
		return k%2 == 0
	})
